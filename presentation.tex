...
\begin{frame}{Presentación}
	...
	\small \url{https://github.com/umoqnier/git-workshop-optics-chapter/}
\end{frame}
...
\begin{frame}{\texttt{\$ whoami}}
	\begin{columns}
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				\item Ingeniero en Computación @ FI, UNAM
				\item Maestrante @ PCIC, UNAM
				\item Colaborador en proyectos de software libre @ Elotl, LIDSoL
				\item Me late la cultura libre, la ciencia ficción y andar en bici sin frenos
			\end{itemize}
		\end{column}

		\begin{column}{0.4\textwidth}
			\centering
			\includegraphics[width=\textwidth]{img/d_low.jpg}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Disclaimer III (La venganza de los Sith)}
\Large Estas herramientas son \textbf{alternativas} a tu flujo de trabajo actual no una sustitución\footnote{A menos que te guste probar cosas \textit{cold turky}}.
\end{frame}
...
\begin{frame}{¿Porqué migrar \textit{Overleaf} $\rightarrow$ \texttt{git}?}
	\begin{itemize}
		...
		\item <4-> Control de versiones de tus documentos grado industrial
		\item <5-> Puedes usar LLM locales y de pago como asistentes de escritura
	\end{itemize}
\end{frame}
...
\begin{frame}{Moviendo archivos entre áreas}
	...
	\begin{alertblock}{Commandos}
		\begin{itemize}
			\item <1-> \texttt{\$ git add} $\rightarrow$ Mueve archivos al área de \alert{staging}\footnote{Esta área es previa a cambios definitivos (\alert{commit})}
			\begin{itemize}
			...
			\end{itemize} 
			\item <2-> \texttt{\$ git commit} $\rightarrow$ Confirma los cambios que esten en el area de \alert{staging}. Se crea un punto en la historia del proyecto.
			\item <3-> \texttt{\$ git restore} $\rightarrow$ Devuelve el archivo al estado original del último \alert{commit}.
			\begin{itemize}
				\item Agregar mensaje de confirmación - \texttt{git commit -m ``Mensaje''}
			\end{itemize}
			\item <4-> \texttt{\$ git restore ---staged} $\rightarrow$ Descarta los cambios del área de \alert{staging}\footnote{Esto es, devolverlos al \alert{working directory}}.
		\end{itemize}
	\end{alertblock}
\end{frame}
...
\begin{frame}
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{img/git_meme.png}
	\end{figure}
\end{frame}
...
\begin{frame}{Resolución de conflictos}
     ...
     Un \alert{conflicto}\footnote{Un conflicto no es un error del sistema, es una solicitud de intervención humana para evitar la pérdida de datos.} ocurre cuando dos personas (o dos ramas) modifican la \textbf{misma línea} del \textbf{mismo archivo}, y \texttt{git} no puede decidir automáticamente cuál versión mantener.
...
\begin{frame}
	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{img/merge_conflict.png}
	\end{figure}
...
\begin{frame}{Latex Workshop}
	Latex workshop es una extensión para \texttt{VS Code} que brinda características convenientes para escribir textos en latex. 
	
	\begin{figure}
		\centering
		\includegraphics[width=0.4\textwidth]{img/latex_workshop.png}
		\caption{\url{https://marketplace.visualstudio.com/items?itemName=James-Yu.latex-workshop}}
	\end{figure}
\end{frame}
...
\begin{frame}{Herramientas}
	\begin{columns}[t] % [t] aligns columns at the top
        ...
        \begin{column}{0.5\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.6\textwidth]{img/ollama.png} \\
                \small \url{https://ollama.com/}
            \end{figure}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.6\textwidth]{img/continue.png}	
                \small \url{https://marketplace.visualstudio.com/items?itemName=Continue.continue}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}[fragile]{Comandos básicos de ollama}
	\begin{verbatim}
		$ ollama serve
...
\begin{frame}[fragile]{Config basica}
...
\begin{frame}{Dónde encontrar ayuda}
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{img/lidsol.png}
		\caption{\url{https://lidsol.unam.mx}}
	\end{figure}

	\begin{itemize}
		\item \href{https://www.youtube.com/playlist?list=PLvd_owpd6H-knKHAikePrqLwMrVje2dY9}{Curso de git}
		\item \href{https://www.youtube.com/playlist?list=PLvd_owpd6H-kSoqbbvdGc_Pg3YRytK8ZN}{Curso de \LaTeX}
		\item \href{https://git-scm.com/book/es/v2}{Libro de git}
	\end{itemize}
\end{frame}
...
\begin{frame}
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{img/fin.jpg}
	\end{figure}	
\end{frame}
